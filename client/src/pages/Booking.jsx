...
const Booking = () => {
    ...
    // --- Data Fetching ---
    // Fetch counselors and the user's existing bookings when the component first loads.
        ...
        if (window.confirm('Are you sure you want to cancel this appointment?')) {
            try {
                ...
                setSuccess('Appointment successfully cancelled.');
                 // Re-fetch bookings to update the list and reflect the change
                 const bookingsRes = await getMyBookings();
                 setMyBookings(bookingsRes.data);
            } catch(err){
    ...
    // --- JSX Rendering ---
    return (
        <div className="container mt-10 mx-auto p-4 sm:p-6 lg:p-8 bg-gray-50 min-h-screen">
            ...
            <div className="bg-white p-8 rounded-xl shadow-lg mb-8 max-w-2xl mx-auto">
                ...
                <form onSubmit={onSubmit} className="space-y-4">
                    ...
                    <button type="submit" disabled={loading} className="w-full py-3 px-4 border border-transparent rounded-md shadow-sm text-sm font-medium text-white bg-blue-600 hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 disabled:bg-blue-300 disabled:cursor-not-allowed">
                 ...
                 <div className="space-y-4">
                    {myBookings.length > 0 ? (
                        myBookings.map(booking => (
                            <div key={booking._id} className="border border-gray-200 p-4 rounded-lg flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                                <div>
                                ...
                                <div className="flex items-center self-end sm:self-center">
                                   <span className={`px-3 py-1 text-xs font-medium rounded-full ${getStatusColor(booking.status)}`}>{booking.status}</span>
                                    {booking.status === 'Scheduled' && (
                                        <button onClick={() => handleCancel(booking._id)} className="ml-4 text-sm font-medium text-red-600 hover:underline">Cancel</button>
                                    )}
                                </div>
                            </div>
                        ))
                    ) : (
                        <p className="text-center text-gray-500 py-4">You have no upcoming or past appointments.</p>
                    )}
                 </div>
            </div>
