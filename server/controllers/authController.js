...
export const registerUser = asyncHandler( async ( req, res) =>{
  ...
  if([name,email, password ,role].some((field)=>
    ...
    if(user.role == 'counselor'){
        throw new ApiError(400, "go to the Counselor registration , You can only register a counslor  in counselor regisration section")
    }
...
//   }
  ...
  const { name, email, password, specializations, bio, availability } = req.body;
